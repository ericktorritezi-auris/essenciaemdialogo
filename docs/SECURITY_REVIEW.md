...
- Colaborador não consegue acessar rotas Admin-only (`/admin/home`, `/admin/menu`, `/admin/platforms`, `/admin/radio`, gestão de plataformas) — a checagem está lá (`requireAdmin()`), mas isso não foi testado *ao vivo* com uma conta Colaborador de verdade, porque não tenho como criar/logar como Colaborador a partir daqui. **Recomendo que você teste isso manualmente** com a conta de Colaborador de teste (ver `docs/USER_MANAGEMENT.md`) — é o item mais importante da checklist de QA abaixo.
